...
class Linear(object):
    ...
    def forward(x, w, b):
        ...
        # reshape the tensor
        x2 = x.reshape(N, D)
        #  matrix multiplication of x2 to w + b
        # out = x2@(w) + b
...
def softmax_loss(x, y):
    ...
    # Compute softmax
    dx = torch.softmax(x, dim=1 )
    # Compute cross entropy loss
    # loss = torch.nn.functional.cross_entropy(x, y)
    loss = -torch.sum(torch.log(dx[range(x.shape[0]), y])) / x.shape[0]
    # Compute gradient
    # subtracts 1 from predicted probabilities of the true classes for each sample
...
class TwoLayerNet(object):
    ...
    def __init__(self, input_dim=3*32*32, hidden_dim=100, num_classes=10,
        ...
        self.params['W1'] = torch.zeros(input_dim , hidden_dim,device=device, dtype=dtype).normal_(mean = 0,std=weight_scale)
        self.params['b1'] = torch.zeros(hidden_dim,device=device, dtype=dtype)
        self.params['W2'] = torch.zeros(hidden_dim , num_classes,device=device, dtype=dtype).normal_(mean = 0,std=weight_scale)
    ...
    def loss(self, X, y=None):
        ...
        # h1 = torch.relu(X.mm(W1)+b1)
        # scores = h1.mm(W2) + b2

        h1 , cache_LR1 = Linear_ReLU.forward(X ,W1,b1)
        scores, cache_L2 = Linear.forward(h1,W2,b2)
        ...
        dy2, grads['W2'] , grads['b2'] = Linear.backward(softmax,cache_L2)
        dy1, grads['W1'] , grads['b1'] = Linear_ReLU.backward(dy2,cache_LR1)

        grads['W2'] += 2*self.reg*W2
        grads['W1'] += 2*self.reg*W1
        
        L2_reg_loss =self.reg * (torch.sum((W1 ** 2)) + torch.sum((W2 ** 2)))
        loss += L2_reg_loss
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        self.params['W1'] = torch.zeros(input_dim , hidden_dims[0],device=device, dtype=dtype).normal_(mean = 0,std=weight_scale)
        self.params['b1'] = torch.zeros(hidden_dims[0],device=device, dtype=dtype)
        # self.num_layers = len(hidden_dims)+1
        for L in range(2, len(hidden_dims)+1):
          self.params[f'W{L}'] = torch.zeros(hidden_dims[L-2] , hidden_dims[L-1],device=device, dtype=dtype).normal_(mean = 0,std=weight_scale)
          self.params[f'b{L}'] = torch.zeros(hidden_dims[L-1],device=device, dtype=dtype)
          
        self.params[f'W{len(hidden_dims)+1}'] = torch.zeros(hidden_dims[len(hidden_dims)-1] ,num_classes ,device=device, dtype=dtype).normal_(mean = 0,std=weight_scale)
        self.params[f'b{len(hidden_dims)+1}'] = torch.zeros(num_classes,device=device, dtype=dtype)
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1, self.num_layers ):
            ...
            # H = torch.relu(H.mm(W)+b)
            H , cache= Linear_ReLU.forward(H,W,b)
            # print('cache_f',len(cache))
            Relu_outputs[f'H{i}'] = H
            cache_out[f'c{i}'] = cache

        Wn , bn= self.params[f'W{i+1}'] , self.params[f'b{i+1}']

        # scores = H.mm(Wn ) + bn
        scores ,cache= Linear.forward(H,Wn,bn)
        cache_out[f'c{i+1}'] = cache
        # print('cache_f',len(cache))
        # print('Relu_outputs:',Relu_outputs.keys())
        #################################################################
        #                      END OF YOUR CODE                         #
        #################################################################
        ...
        # Replace "pass" statement with your code
        loss , softmax = softmax_loss(scores,y)
        num_weights =self.num_layers

        L2_reg_loss = self.reg * torch.sum((self.params[f'W{num_weights}']**2))
        for Wi in range(num_weights-1,0,-1):
            L2_reg_loss += self.reg * torch.sum((self.params[f'W{Wi}']**2))
             
        loss += L2_reg_loss/2

        
        dx, grads[f'W{num_weights}'] , grads[f'b{num_weights}'] = Linear.backward(softmax,cache_out[f'c{num_weights}'])
        grads[f'W{num_weights}'] += self.reg* self.params[f'W{num_weights}']
        
        for i in range(num_weights-1,0 ,-1):
          dx, grads[f'W{i}'] , grads[f'b{i}'] =  Linear_ReLU.backward(dx,cache_out[f'c{i}'])
...
def create_solver_instance(data_dict, dtype, device):
    ...
    print(f'{device} is the current device.')
...
def get_five_layer_network_params():
    ...
    # model achieves 100% training accuracy within 20 epochs.      #
    ################################################################
    weight_scale = 0.25   # Experiment with this!
    learning_rate = 0.0025  # Experiment with this!
    # we get the same accuracy on train data(100% or better to 
    # say overfitting) with half of these paramiters value
    ################################################################
    #                       END OF YOUR CODE                       #
    ################################################################
    return weight_scale, learning_rate
...
def adam(w, dw, config=None):
    ...
    # Update the first moment estimate with the beta1 and the current gradient
    config['m'] = config['beta1'] * config['m'] + (1 - config['beta1']) * dw

    # Update the second moment estimate with the beta2 and the current gradient
    ...
    # Update the next value of w with the learning rate, the bias-corrected estimates, and the epsilon value
